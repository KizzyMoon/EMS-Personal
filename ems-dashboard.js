...
const DEFAULT_MY_EMPLOYEE_NUMBER = "";
...
function normalizeSettings(raw = {}) {
  return {
    ...
    myEmployeeNumber: normalizeEmployeeNumber(raw.myEmployeeNumber || DEFAULT_MY_EMPLOYEE_NUMBER),
...
function trainingCell(rows, rowNumber, columnNumber) {
...
function isUpcomingTraining(dateText) {
...
function employeeMatch(employeeNumber) {
  const key = normalizeEmployeeNumber(employeeNumber);
  if (!key) return null;
  return state.members.find((member) => normalizeEmployeeNumber(member.employeeNumber) === key)
    || state.cadets.find((cadet) => normalizeEmployeeNumber(cadet.employeeNumber) === key)
    || null;
}

function trainingPerson(employeeNumber, fallbackName, section) {
  const match = employeeMatch(employeeNumber);
  const tags = match?.tags || [];
  const rank = match?.rank || (section === "cadet" ? "Cadet" : "");
  let tag = "HELPER";
  if (section === "cadet") tag = "CADET";
  else if (tags.some((item) => normalizeKey(item) === "fto")) tag = "FTO";
  else if (["chief", "deputychief", "captain", "lieutenant", "sergeant"].includes(normalizeKey(rank))) tag = "SUPERVISOR";
  else if (normalizeKey(rank) === "cadet") tag = "CADET";

  return {
    employeeNumber: normalizeEmployeeNumber(employeeNumber),
    name: match?.name || fallbackName || `Employee ${employeeNumber}`,
...
function trainingSignupRows(rows, employeeColumn, nameColumn, startRow, endRow, section) {
  const results = [];
  for (let row = startRow; row <= endRow; row += 1) {
    const employeeNumber = trainingCell(rows, row, employeeColumn);
    if (!normalizeEmployeeNumber(employeeNumber)) continue;
    results.push(trainingPerson(employeeNumber, trainingCell(rows, row, nameColumn), section));
  }
  return results;
}

function parseTrainingBlock(rows, config) {
  const title = trainingCell(rows, 8, config.employeeColumn) || `EU Day ${config.day} Training`;
  const host = trainingCell(rows, 11, config.employeeColumn);
  const date = parseTrainingDate(trainingCell(rows, 11, config.nameColumn));
  const time = trainingCell(rows, 11, config.timeColumn);
  const cadets = trainingSignupRows(rows, config.employeeColumn, config.nameColumn, 13, 26, "cadet");
  const staff = trainingSignupRows(rows, config.employeeColumn, config.nameColumn, 29, 55, "staff");
  const myEmployeeNumber = normalizeEmployeeNumber(state.settings?.myEmployeeNumber);
  const signedUp = Boolean(myEmployeeNumber) && [...cadets, ...staff].some(
  ...
  const eligibleNumbers = new Set(eligibleCadets.map((cadet) => normalizeEmployeeNumber(cadet.employeeNumber)).filter(Boolean));
  const eligibleSignedUp = eligibleNumbers.size
    ? cadets.filter((person) => eligibleNumbers.has(person.employeeNumber)).length
    : cadets.length;
...
function parseTrainingRows(rows) {
  return [
    parseTrainingBlock(rows, { day: 1, employeeColumn: 2, nameColumn: 3, timeColumn: 4 }),
    parseTrainingBlock(rows, { day: 2, employeeColumn: 12, nameColumn: 13, timeColumn: 14 })
  ].filter((session) => isUpcomingTraining(session.date));
}

function trainingPersonHtml(person) {
  const identity = [person.callsign, person.name].filter(Boolean).join(" | ");
  return `
    <li class="training-person">
      <span class="training-role training-role-${escapeHtml(person.tag.toLowerCase())}">${escapeHtml(person.tag)}</span>
      <span>${escapeHtml(identity || person.employeeNumber)}</span>
    </li>
  `;
}

function trainingHostHtml(session) {
  if (!session.host) return `<span class="muted">Not entered yet</span>`;
  const byEmployee = employeeMatch(session.host);
  if (byEmployee) {
    return escapeHtml([byEmployee.callsign, byEmployee.name].filter(Boolean).join(" | "));
  }
  return escapeHtml(session.host);
}

function trainingCardHtml(session) {
  const hasEmployeeNumber = Boolean(normalizeEmployeeNumber(state.settings?.myEmployeeNumber));
  const badgeClass = session.signedUp ? "training-signup-good" : "training-signup-bad";
  const badgeText = session.signedUp ? "SIGNED UP" : "NOT SIGNED UP";
  const countText = session.eligibleTotal
    ? `${session.eligibleSignedUp} / ${session.eligibleTotal} cadets signed up`
    : `${session.cadets.length} cadet${session.cadets.length === 1 ? "" : "s"} signed up`;
  return `
    <article class="training-card">
      <span class="training-signup ${badgeClass}">${hasEmployeeNumber ? badgeText : "SET EMPLOYEE #"}</span>
      <div class="training-card-head">
        <div>
          <span class="training-day">EU DAY ${session.day}</span>
          <h3>Training Day ${session.day}</h3>
          <p class="training-date">${escapeHtml(formatDate(session.date))}${session.time ? ` • ${escapeHtml(session.time)}` : ""}</p>
        </div>
      </div>
      <div class="training-summary">
        <div>
          <span>Hosted by</span>
          <strong>${trainingHostHtml(session)}</strong>
        </div>
        <div>
      ...
      <div class="training-lists">
        <section>
          <h4>Cadets</h4>
          ${session.cadets.length ? `<ul>${session.cadets.map(trainingPersonHtml).join("")}</ul>` : `<p class="muted">No cadets signed up yet.</p>`}
        </section>
        <section>
          <h4>Supervisors, FTOs & Helpers</h4>
          ${session.staff.length ? `<ul>${session.staff.map(trainingPersonHtml).join("")}</ul>` : `<p class="muted">No staff signed up yet.</p>`}
        </section>
      </div>
    </article>
  `;
...
function renderTraining() {
  ...
  els.trainingList.innerHTML = liveTrainingSessions.length
    ? liveTrainingSessions.map(trainingCardHtml).join("")
    : empty("There are no upcoming EU Day 1 or Day 2 training dates on the sheet.");
}

async function fetchPublicTrainingRows() {
  const url = `${googleCsvUrl(state.settings?.trainingUrl || DEFAULT_TRAINING_URL)}&cacheBust=${Date.now()}`;
  const response = await fetch(url, { cache: "no-store" });
  if (!response.ok) throw new Error(`Google returned ${response.status}.`);
  const text = await response.text();
  if (/<!doctype|<html|sign in/i.test(text.slice(0, 500))) {
    throw new Error("The Training Attendance Sheet did not return live CSV. Google sign-in may be required.");
  }
  return parseCsvRows(text);
}

async function fetchPrivateTrainingRows() {
  const { id, gid } = sheetInfoFromUrl(state.settings?.trainingUrl || DEFAULT_TRAINING_URL);
  const title = await sheetTitleFromInfo(id, gid);
  const range = encodeURIComponent(`'${title.replace(/'/g, "''")}'!A1:T63`);
  const values = await fetchSheetJson(`https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(id)}/values/${range}?majorDimension=ROWS&valueRenderOption=FORMATTED_VALUE`);
...
async function refreshTraining(options = {}) {
  ...
  try {
    let rows;
    try {
      rows = await fetchPublicTrainingRows();
    } catch (publicError) {
      if (!options.allowGoogleSignIn) throw publicError;
      rows = await fetchPrivateTrainingRows();
    }
    liveTrainingSessions = parseTrainingRows(rows);
    trainingLoadState = "ready";
    trainingLoadMessage = `Live sheet refreshed ${new Date().toLocaleString("en-GB")}`;
  } catch (error) {
...
async function importGoogleSheet() {
  ...
  await refreshTraining({ allowGoogleSignIn: true });
  if (errors.length) {
    alert(`Synced with issues.\n\nImported ${cadetCount} cadet row(s) and ${rosterCount} roster row(s).\n\n${errors.join("\n")}`);
  } else {
    alert(`Synced ${cadetCount} cadet row(s), ${rosterCount} roster row(s), and refreshed upcoming EU training.`);
  }
}
...
function renderSettings() {
  ...
  if (els.settingsSummary) {
    const employeeText = state.settings?.myEmployeeNumber || "not set";
    els.settingsSummary.textContent = `RA callsign: ${state.settings?.myCallsign || DEFAULT_MY_CALLSIGN} • Employee number: ${employeeText}`;
  }
}
...
function saveSettings() {
  ...
  saveState();
  render();
  refreshTraining({ allowGoogleSignIn: false });
  alert(`Settings saved. Employee number: ${state.settings.myEmployeeNumber || "not set"}.`);
}
...
document.addEventListener("click", async (event) => {
  ...
  if (action === "refresh-training") refreshTraining({ allowGoogleSignIn: true });
